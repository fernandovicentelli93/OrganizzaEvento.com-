...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return generateLocalizedMetadata({ params: Promise.resolve({ locale: "en", path: ["questions", safeDecode(slug)] }) });
}

export default async function EnglishLocalizedConversationPage({ params }: PageProps) {
  const { slug } = await params;
