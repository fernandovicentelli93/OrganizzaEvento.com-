...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return generateLocalizedMetadata({ params: Promise.resolve({ locale: "es", path: ["questions", safeDecode(slug)] }) });
}

export default async function SpanishLocalizedConversationPage({ params }: PageProps) {
  const { slug } = await params;
