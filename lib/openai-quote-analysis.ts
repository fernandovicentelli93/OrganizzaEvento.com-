...
function promptForQuote(input: OpenAIQuoteAnalysisInput) {
  ...
  return [
    ...
    "The public summary must be one coherent post, not multiple drafts pasted together. Never repeat the opening greeting, never repeat the same offer summary, and never write two versions of the same sentence.",
    "The public summary must have 3 short paragraphs: 1) what quote the user received and the visible context, 2) what seems included and what is still unclear, 3) 2 or 3 concrete questions for the community.",
    "Do not write labels like 'Included items:', 'Unclear points:' or bullet lists inside public_anonymized_summary. It must read like a human forum post.",
    "Use complete, natural sentences in the requested language only. Avoid mixed languages, broken punctuation, duplicated clauses, malformed accents and machine-like transitions.",
    "Base every question on details actually present or strongly implied in the quote text/OCR/context: service type, duration, guest count, city, amount, VAT, travel, setup, equipment, staff, timing, cancellation, plan B or extras.",
    "If OCR text is noisy or incomplete, say calmly that some lines are hard to read and ask what should be clarified, instead of inventing details.",
    "For Italian, use natural Italian: 'Ciao, ho ricevuto...', 'Mi chiedo se...', 'Secondo voi...'. Do not write 'per altro' unless the quote truly has no identifiable theme.",
    "For English, Spanish and French, write from the perspective of someone comparing the quote with Italian clients and suppliers before confirming an event in Italy.",
    "Questions_to_ask must be specific and different from each other. Avoid generic questions unless the quote is truly empty.",
